...
# - EntityRuler: For rule-based skill matching (used to initialize the entity_ruler pipeline component)
...
# - Allow requests from localhost:3000 (development) and production domain
# - Permit all methods (GET, POST, etc.) and headers for flexibility
app.add_middleware(
    CORSMiddleware,
...
# - Use 'en_core_web_md' for medium-sized model with word embeddings
# - Disable 'ner' to reduce processing time, keep 'parser' for noun chunks and enable lemmatizer
try:
...
# - Convert each skill name into a spaCy Doc to avoid re-processing during extraction
...
# - Add EntityRuler pipeline component before textcat (if not already present)
# - Create patterns for known skills from skills.json and dynamic technical term patterns
# - Type hint clarifies EntityRuler usage to suppress Pylance warning
ruler: EntityRuler = nlp.add_pipe("entity_ruler", before="textcat") if "entity_ruler" not in nlp.pipe_names else nlp.get_pipe("entity_ruler")
skill_patterns = [
    ...
    {"label": "SKILL", "pattern": [{"TEXT": {"REGEX": r"^[A-Z][a-z]+[A-Z][a-z]+"}}]},  # CamelCase (e.g., JavaScript)
    {"label": "SKILL", "pattern": [{"TEXT": {"REGEX": r"^[A-Z]{2,}$"}}]},  # Acronyms (e.g., AWS, CI/CD)
    {"label": "SKILL", "pattern": [{"TEXT": {"REGEX": r"^[a-zA-Z]+\.[a-zA-Z]+"}}]},  # Dot notation (e.g., Node.js)
    {"label": "SKILL", "pattern": [{"TEXT": {"REGEX": r"^[a-zA-Z]+-[a-zA-Z]+"}}]},  # Hyphenated terms (e.g., CI-CD)
    {"label": "SKILL", "pattern": [{"POS": "PROPN", "OP": "+"}, {"POS": "NOUN", "OP": "*"}]},  # Multi-word proper nouns (e.g., machine learning)
    {"label": "SKILL", "pattern": [{"LOWER": {"IN": ["graphql", "langchain", "vite", "nextjs"]}}]}  # Common lowercase technical terms
]
ruler.add_patterns(skill_patterns)

# Define Pydantic model for incoming job description data
# - Expect a JSON payload with a 'description' field (string)
...
# - Output: List of extracted skill names
...
def extract_skills_single(description: str) -> List[str]:
    ...
    for ent in doc.ents:
        if ent.label_ == "SKILL":
            # Ensure the skill is in a requirement context (e.g., "requires GraphQL")
    ...
    for chunk in doc.noun_chunks:
        ...
        if lemma not in extracted_skills and len(lemma) > 2:
            ...
            if (
                ...
                any(word.isupper() for word in chunk_text.split()) or  # Acronyms
    ...
    # Return the list of unique extracted skills
