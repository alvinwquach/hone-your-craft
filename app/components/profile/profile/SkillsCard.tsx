...
function SkillsCard({ userSkills }: SkillsCardProps) {
  ...
  return (
    <div className="bg-neutral-900 flex flex-col lg:flex-row gap-8 p-6 sm:p-8 mt-4 sm:mt-0 border border-zinc-700">
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-2 mb-4">
            {selectedSkills.map((skill) => (
              <div
                key={skill}
                className="bg-neutral-800 text-white px-3 py-1 text-sm inline-flex items-center gap-2"
              >
                {skill}
                <button
          ...
          <Combobox as="div" value={query} onChange={setQuery}>
            <Combobox.Input
              ...
              className="block w-full p-4 text-sm border rounded-lg bg-neutral-800 text-white focus:ring-blue-500 focus:border-blue-500 border-zinc-700 placeholder-gray-400"
              placeholder="Select skill"
            />
            {filteredSkills.length > 0 && (
              <Combobox.Options className="mt-2 bg-neutral-800 border border-zinc-700 text-white rounded-lg max-h-48 overflow-y-auto p-2 w-full">
